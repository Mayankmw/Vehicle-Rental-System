...
<body>
    ...
    <script>alert('Booking Request Sent to Admin..!');</script>
    <%}%>
    <%
    if (request.getParameter("msgg") != null) {%>
    <script>alert('Vehicle Already Booked..!');</script>
    <%}%>
    ...
    <li class="current_page_item"><a href="cviewvehicles.jsp">View Vehicles</a></li>
    ...
    <tr>
        ...
        <td><a href="cbook.jsp?vtype=<%=rs.getString(2)%>&vname=<%=rs.getString(3)%>&vno=<%=rs.getString(4)%>&capacity=<%=rs.getString(5)%>&cost=<%=rs.getString(6)%>"><button type="button">Book</button></a></td>
